...
(define (make-inc inst machine labels pc)
  (define (inc-reg-name instruction) (cadr instruction))
  (let ((reg (get-register machine (inc-reg-name inst)))
        (exc (lookup-label labels 'exception)))
    (lambda ()
      (let ((contents (get-contents reg)))
        (if (number? contents)
            (begin (set-contents! reg (+ contents 1))
                   (advance-pc pc))
            (set-contents! pc exc))))))
            
(define (make-dec inst machine labels pc)
  (define (dec-reg-name instruction) (cadr instruction))
  (let ((reg (get-register machine (dec-reg-name inst)))
        (exc (lookup-label labels 'exception)))
    (lambda ()
      (let ((contents (get-contents reg)))
        (if (number? contents)
...
(define (make-machine-from-text machine-text)
  ...
  (define exception (cadr (caddddr machine-text)))
  (let ((new-controller
        (append controller
                (list
                 (list 'goto (list 'label 'exitpnt))
                 'exception)
                exception
                (list 'exitpnt))))
    ;(displayln new-controller)
    (make-machine reg-table op-table new-controller)))
