...
(define (eval exp env)
  (cond ((self-evaluating? exp ) exp)
        ...
        ((class? exp) (eval-class exp env))
        ((new? exp) (eval-new exp env))
        ((instance? (eval (operator exp) env))
         (eval-instance exp env))
...
(define (eval-until-instance exp env)
  (cond ((self-evaluating? exp ) exp)
        ((null? exp) (void))
        ((variable? exp) (lookup-variable-value exp env))
        ((quoted? exp) (text-of-quotation exp))
        ...
        ((new? exp) (eval-new exp env))
        ((instance? (eval (operator exp) env))
         (cons (eval (operator exp) env) (cdr exp)))
...
(define (eval-class exp env)
  (let ((class-name (cadr exp))
        (class-contents (cddr exp)))
    (define-variable! class-name class-contents env)))
(define (new? exp) (tagged-list? exp 'new))
(define (eval-new exp env)
  (let ((class-name (cadr exp)))
    (let ((class-contents (lookup-variable-value class-name env)))
      (let ((extended-env (extend-environment (mlist) (mlist) env)))
        (for-each (lambda (e) (eval e extended-env)) class-contents)
        (cons 'instance extended-env)))))
(define (instance? exp) (tagged-list? exp 'instance))
(define (eval-instance exp env)
  (lookup-variable-value (cadr exp) (cdr (eval (car exp) env))))
...
(define (eval-assignment exp env)
  (let ((var (assignment-variable exp))
    ...
    (if (symbol? var) ; normal variable
        (set-variable-value! (assignment-variable exp)
                             val
                             env)
        (let ((new-exp (eval-until-instance var env)))
          (set-variable-value! (cadr new-exp)
                               val
                               (cdr (car new-exp)))))
    (void)))
